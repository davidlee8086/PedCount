...
void VehicleCouting::process()
{
  ...
  if(FAV1::use_roi == true && FAV1::roi_defined == false && firstTime == true)
  {
    do
    {
      ...
      cv::imshow("PedestrianCounting", img_input);
	...
	if (points.count(id) > 0)
	{
		...
		if (track->inactive == 0 && centroids.size() < 30)
		{
			...
			for (std::vector<CvPoint2D64f>::iterator it3 = centroids.begin(); it3 != centroids.end(); it3++)
			{
				...
				std::string label = "id=" + std::to_string(int(id)) + " (" + std::to_string(int(centroid.x)) + "," + std::to_string(int(centroid.y)) + ")";
				cv::putText(img_input, label, cv::Point(int(centroid.x) + 20, int(centroid.y) - 20), cv::FONT_HERSHEY_COMPLEX_SMALL, 1, cv::Scalar(0, 0, 255), 2);
		...
		if (current_position != old_position){
			if ((centroid.x >= FAV1::roi_x0) && (centroid.x <= FAV1::roi_x1)){
				if (old_position == VP_A && current_position == VP_B){
					...
					cv::putText(img_input, xing, cv::Point(1100, img_h - 180), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0, 0, 255), 2);
  ...
  if(showAB == 0)
  {
	...
	cv::putText(img_input, countABstr, cv::Point(1100, img_h - 100), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0,0,255), 2);
	cv::putText(img_input, countBAstr, cv::Point(1100, img_h - 60), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0,0,255), 2);
  }
  // Only Display A->B
  if (showAB == 1){
	  cv::putText(img_input, countABstr, cv::Point(1100, img_h - 100), cv::FONT_HERSHEY_PLAIN, 2, cv::Scalar(0, 0, 255), 2);
